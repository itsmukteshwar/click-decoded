/* eslint-disable */
// src/app/industries/finance/page.tsx — generated from industry-finance.html (do not edit header/footer here)
import type { Metadata } from 'next'
import PageScript from '@/components/chrome/PageScript'

export const metadata: Metadata = {
  title: "Digital Marketing for Financial Services & NBFCs India | Click",
  description: "Lead generation, SEO, and compliant digital marketing for banks, NBFCs, insurance companies, and financial advisors in India. Click Decoded drives quality.",
  alternates: { canonical: "/industries/finance" },
}

const css = `
:root{--n:#2A4573;--n2:#1e3460;--n-dark:#0f1e38;--o:#EE7E1A;--o2:#d46e12;--body:#1E293B;--m:#475569;--s:#64748b;--bg:#F4F7FC;--bdr:#E2E8F0;--gr:#25D366;--ac:#1976D2;--ac-rgb:25,118,210;}
...
.hero::before{content:'';position:absolute;inset:0;background:radial-gradient(ellipse 55% 60% at 70% 30%,rgba(25,118,210,.18) 0%,transparent 65%),radial-gradient(ellipse 35% 40% at 5% 80%,rgba(25,118,210,.07) 0%,transparent 60%);pointer-events:none;}
.hero-grid{display:grid;grid-template-columns:1fr 1fr;gap:60px;align-items:center;position:relative;z-index:2;}
.hero-eyebrow{display:inline-flex;align-items:center;gap:8px;padding:6px 16px;background:rgba(25,118,210,.12);border:1px solid rgba(25,118,210,.3);border-radius:99px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.12em;color:#90caf9;margin-bottom:20px;}
...
.hero h1 em{font-style:normal;background:linear-gradient(135deg,#90caf9,#1976D2);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;}
.hero-desc{font-size:17px;color:rgba(255,255,255,.62);line-height:1.7;margin-bottom:32px;max-width:460px;}
.hero-btns{display:flex;gap:12px;flex-wrap:wrap;margin-bottom:36px;}
.fin-mock{background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.1);border-radius:20px;overflow:hidden;backdrop-filter:blur(10px);}
.fin-mock-header{background:rgba(255,255,255,.05);border-bottom:1px solid rgba(255,255,255,.08);padding:14px 18px;display:flex;align-items:center;justify-content:space-between;}
.fin-mock-title{font-size:12px;font-weight:700;color:rgba(255,255,255,.7);letter-spacing:.04em;}
.fin-live{display:flex;align-items:center;gap:6px;font-size:11px;color:#1976D2;font-weight:600;}
.fin-live::before{content:'';width:7px;height:7px;border-radius:50%;background:#1976D2;box-shadow:0 0 0 3px rgba(25,118,210,.2);animation:finpulse 2s infinite;}
@keyframes finpulse{0%,100%{box-shadow:0 0 0 3px rgba(25,118,210,.2);}50%{box-shadow:0 0 0 6px rgba(25,118,210,.05);}}
.fin-kpis{display:grid;grid-template-columns:1fr 1fr;gap:1px;background:rgba(255,255,255,.06);}
.fin-kpi{background:#0f1e38;padding:16px 18px;}
.fin-kpi-label{font-size:10.5px;font-weight:600;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.08em;margin-bottom:6px;}
.fin-kpi-val{font-size:22px;font-weight:900;color:#fff;letter-spacing:-.02em;line-height:1;}
.fin-kpi-val.blue{color:#1976D2;}
.fin-kpi-delta{font-size:11px;margin-top:4px;font-weight:600;}
.fin-kpi-delta.up{color:#4caf50;}
.fin-rows{padding:16px 18px;border-top:1px solid rgba(255,255,255,.06);}
.fin-rows-label{font-size:10.5px;font-weight:600;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.08em;margin-bottom:10px;}
.fin-row{display:flex;align-items:center;justify-content:space-between;padding:6px 0;border-bottom:1px solid rgba(255,255,255,.05);font-size:12px;}
.fin-row:last-child{border-bottom:none;}
.fin-row-name{color:rgba(255,255,255,.6);}
.fin-row-val{color:#90caf9;font-weight:700;}
.fin-bottom{padding:14px 18px;background:rgba(25,118,210,.06);border-top:1px solid rgba(25,118,210,.1);display:flex;align-items:center;justify-content:space-between;}
.fin-bottom-label{font-size:12px;color:rgba(255,255,255,.5);font-weight:500;}
.fin-bottom-val{font-size:16px;font-weight:800;color:#1976D2;}
...
@media(max-width:960px){.hero-grid{grid-template-columns:1fr;}.fin-mock{display:none;}.pain-grid,.why-grid{grid-template-columns:repeat(2,1fr);}.faq-grid{grid-template-columns:1fr;}.fg{grid-template-columns:1fr 1fr;}}
...
.hero-btns{display:flex;gap:12px;flex-wrap:wrap;margin-bottom:36px;}
.fin-mock{background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.1);border-radius:20px;overflow:hidden;backdrop-filter:blur(10px);}
.fin-hdr{background:rgba(255,255,255,.05);border-bottom:1px solid rgba(255,255,255,.08);padding:14px 18px;display:flex;align-items:center;justify-content:space-between;}
.fin-hdr-title{font-size:12px;font-weight:700;color:rgba(255,255,255,.7);letter-spacing:.04em;}
.fin-live{display:flex;align-items:center;gap:6px;font-size:11px;color:#1976D2;font-weight:600;}
.fin-live::before{content:'';width:7px;height:7px;border-radius:50%;background:#1976D2;box-shadow:0 0 0 3px rgba(25,118,210,.2);animation:finp 2s infinite;}
@keyframes finp{0%,100%{box-shadow:0 0 0 3px rgba(25,118,210,.2);}50%{box-shadow:0 0 0 6px rgba(25,118,210,.05);}}
.fin-kpis{display:grid;grid-template-columns:1fr 1fr;gap:1px;background:rgba(255,255,255,.06);}
.fin-kpi{background:#0f1e38;padding:14px 16px;}
.fin-kpi-lbl{font-size:10px;font-weight:600;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.08em;margin-bottom:5px;}
.fin-kpi-val{font-size:20px;font-weight:900;color:#1976D2;letter-spacing:-.02em;line-height:1;}
.fin-kpi-d{font-size:10.5px;margin-top:3px;font-weight:600;color:#4caf50;}
.fin-rows{padding:14px 18px;border-top:1px solid rgba(255,255,255,.06);}
.fin-rows-lbl{font-size:10px;font-weight:600;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px;}
.fin-row{display:flex;justify-content:space-between;align-items:center;padding:5px 0;border-bottom:1px solid rgba(255,255,255,.04);font-size:11.5px;}
.fin-row:last-child{border:none;}
.fin-row-n{color:rgba(255,255,255,.55);}
.fin-row-v{color:#90caf9;font-weight:700;font-size:11px;}
.fin-btm{padding:12px 18px;background:rgba(25,118,210,.06);border-top:1px solid rgba(25,118,210,.1);display:flex;align-items:center;justify-content:space-between;}
.fin-btm-lbl{font-size:11.5px;color:rgba(255,255,255,.45);}
.fin-btm-val{font-size:15px;font-weight:800;color:#1976D2;}
@media(max-width:900px){.hero-grid{grid-template-columns:1fr;}.fin-mock{display:none;}}
...
const ld0 = `{
  ...
  "@graph": [
    {
      "@type": "Service",
      "name": "Digital Marketing for Finance & BFSI",
      "provider": {
        "@type": "Organization",
        "name": "Click Decoded",
        "url": "https://www.clickdecoded.com"
      },
      "description": "Lead generation, SEO, and compliant digital marketing for banks, NBFCs, insurance companies, and financial advisors in India. Click Decoded drives quality financial enquiries.",
      "areaServed": {
        "@type": "Country",
        "name": "India"
      },
      "serviceType": "Digital Marketing for Finance & BFSI",
      "url": "https://www.clickdecoded.com/industries/finance"
    },
    {
      "@type": "BreadcrumbList",
      "itemListElement": [
        ...
        },
        {
          ...
          "position": 2,
          "name": "Digital Marketing for Finance & BFSI",
          "item": "https://www.clickdecoded.com/industries/finance"
        }
      ]
    }
  ]
...
<div>
<div className="hero-eyebrow">💳 Financial Services &amp; NBFC Marketing</div>
<h1>More Quality Leads<br /><em>for Financial Services &amp; NBFCs.</em></h1>
<p className="hero-desc">Financial services is one of the most competitive digital marketing verticals. We know the compliance rules, the buyer psychology, and how to generate verified, high-intent leads.</p>
...
<span className="proof-item"><strong>High-Intent</strong> Verified Leads Only</span>
<span className="proof-item"><strong>Compliant</strong> RBI-Safe Campaigns</span>
<span className="proof-item"><strong>4×</strong> Avg. Lead Quality Score</span>
</div>
</div>
<div className="fin-mock">
<div className="fin-hdr">
<span className="fin-hdr-title">Lead Quality Dashboard</span>
<span className="fin-live">Live</span>
</div>
<div className="fin-kpis">
<div className="fin-kpi"><div className="fin-kpi-lbl">Monthly Leads</div><div className="fin-kpi-val">428</div><div className="fin-kpi-d">↑ 312% in 6 months</div></div>
<div className="fin-kpi"><div className="fin-kpi-lbl">Lead Quality Score</div><div className="fin-kpi-val">8.4/10</div><div className="fin-kpi-d">↑ from 3.1/10</div></div>
<div className="fin-kpi"><div className="fin-kpi-lbl">Cost Per Lead</div><div className="fin-kpi-val">₹180</div><div className="fin-kpi-d">↓ from ₹920</div></div>
<div className="fin-kpi"><div className="fin-kpi-lbl">Loan Disbursals</div><div className="fin-kpi-val">₹4.2Cr</div><div className="fin-kpi-d">↑ attributed to digital</div></div>
</div>
<div className="fin-rows">
<div className="fin-rows-lbl">Top Performing Campaigns</div>
<div className="fin-row"><span className="fin-row-n">Home Loan Search Ads</span><span className="fin-row-v">148 leads · ₹210 CPL</span></div>
<div className="fin-row"><span className="fin-row-n">Mutual Fund — Meta Ads</span><span className="fin-row-v">124 leads · ₹160 CPL</span></div>
<div className="fin-row"><span className="fin-row-n">Business Loan — SEO</span><span className="fin-row-v">92 leads · ₹0 CPL</span></div>
</div>
<div className="fin-btm">
<span className="fin-btm-lbl">💰 Loan Value Attributed to Digital</span>
<span className="fin-btm-val">₹4.2Cr/month</span>
</div>
</div>
</div>
</div>
...
<div className="res-stat"><div className="res-num">3x</div><div className="res-lbl">More Qualified Leads</div></div><div className="res-stat"><div className="res-num">60%</div><div className="res-lbl">Lower Cost Per Lead</div></div><div className="res-stat"><div className="res-num">SEBI</div><div className="res-lbl">Compliant Campaigns</div></div><div className="res-stat"><div className="res-num">₹25K</div><div className="res-lbl">Starting / Month</div></div>
</div>
</div>
...
<div className="pain-card"><div className="pain-icon">📋</div><h3>Lead Quality is Poor</h3><p>Financial product leads from aggregators are expensive and poorly qualified. We drive direct enquiries from people actively searching for your specific product — loan, insurance, or investment.</p></div><div className="pain-card"><div className="pain-icon">🔒</div><h3>Regulatory Constraints on Advertising</h3><p>SEBI, IRDAI, and RBI have strict advertising guidelines. Most agencies don't know the rules and create compliance risks. We navigate these proactively.</p></div><div className="pain-card"><div className="pain-icon">🤝</div><h3>Difficulty Reaching HNI Clients</h3><p>High-net-worth investors and business owners don't respond to mass ads. LinkedIn targeting, referral systems, and thought leadership content reach this segment effectively.</p></div><div className="pain-card"><div className="pain-icon">🔍</div><h3>No Organic Search Presence</h3><p>People search 'business loan in [city]' or 'term insurance vs whole life' millions of times a month. Without SEO, you miss all of this free, intent-driven traffic.</p></div><div className="pain-card"><div className="pain-icon">🤖</div><h3>Slow Lead Response</h3><p>In financial services, the first responder wins. WhatsApp automation that responds to enquiries within 60 seconds dramatically improves conversion rates.</p></div><div className="pain-card"><div className="pain-icon">📊</div><h3>No CRM Integration</h3><p>Financial leads need nurturing over weeks or months. We integrate marketing with your CRM so every lead is properly sequenced — not left to go cold.</p></div>
</div>
</div>
</section>
...
<div className="svc-pill"><a href="/services/seo">🔍 SEO Services</a></div><div className="svc-pill"><a href="/services/google-ads">💰 Google Ads</a></div><div className="svc-pill"><a href="/services/linkedin-ads">💼 LinkedIn Ads</a></div><div className="svc-pill"><a href="/services/blogging">✍️ Blogging &amp; Content</a></div><div className="svc-pill"><a href="/services/whatsapp-automation">💬 WhatsApp Automation</a></div><div className="svc-pill"><a href="/services/email-marketing">✉️ Email Marketing</a></div><div className="svc-pill"><a href="/services/web-development">🌐 Web Development</a></div><div className="svc-pill"><a href="/services/landing-pages">📋 Landing Pages</a></div><div className="svc-pill"><a href="/services/retargeting">🔄 Retargeting</a></div><div className="svc-pill"><a href="/services/ai-chatbot-development">🤖 AI Chatbot Dev</a></div>
</div>
</div>
</section>
...
<div className="faq-item"><button className="faq-q">Do you understand financial advertising regulations?<svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M4 6l4 4 4-4" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" /></svg></button><div className="faq-a"><p>Yes. We're familiar with SEBI, IRDAI, and RBI advertising guidelines. Every campaign is reviewed for compliance before launch — no risk statements missing, no banned claims.</p></div></div><div className="faq-item"><button className="faq-q">Can you help an IFA or independent financial advisor?<svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M4 6l4 4 4-4" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" /></svg></button><div className="faq-a"><p>Yes. Individual financial advisors benefit most from local SEO, LinkedIn thought leadership, and a professional website that builds trust with prospective clients.</p></div></div><div className="faq-item"><button className="faq-q">What financial products do you work with?<svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M4 6l4 4 4-4" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" /></svg></button><div className="faq-a"><p>Personal and business loans, home loans, insurance (life, health, general), mutual funds, wealth management, NBFCs, fintech apps, and credit products.</p></div></div><div className="faq-item"><button className="faq-q">How do you handle lead data in financial services?<svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M4 6l4 4 4-4" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" /></svg></button><div className="faq-a"><p>All lead data is handled securely, stored only in your CRM or our approved tools, and never shared. We use HTTPS forms, secure webhooks, and GDPR-compatible practices.</p></div></div><div className="faq-item"><button className="faq-q">Can you run Google Ads for financial services?<svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M4 6l4 4 4-4" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" /></svg></button><div className="faq-a"><p>Yes. Financial services ads require Google's Financial Products Certification — we are certified. All campaigns include required disclaimers.</p></div></div><div className="faq-item"><button className="faq-q">What's the minimum engagement?<svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M4 6l4 4 4-4" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" /></svg></button><div className="faq-a"><p>Financial services digital marketing starts at ₹25,000/month. Highly competitive verticals (home loans, insurance) may require a higher minimum to generate meaningful data.</p></div></div>
</div>
</div>
</section>
...
<a href="/services/seo" className="rel-pill">SEO Services</a><a href="/services/google-ads" className="rel-pill">Google Ads</a><a href="/services/linkedin-ads" className="rel-pill">LinkedIn Ads</a><a href="/services/whatsapp-automation" className="rel-pill">WhatsApp Automation</a>
</div>
</div>
...
<a href="https://wa.me/919407000101" className="btn btn-wa">💬 WhatsApp Now</a>
      ...
      <PageScript id="industries-finance-0" code={js0} />
    </>
  )
}
